...
const popularAreas = [
  { name: 'CCTV Dubai Marina',       href: '/cctv-installation-dubai-marina' },
  { name: 'CCTV Palm Jumeirah',      href: '/cctv-installation-palm-jumeirah' },
  { name: 'CCTV Downtown Dubai',     href: '/cctv-installation-downtown-dubai' },
  { name: 'CCTV Business Bay',       href: '/cctv-installation-business-bay' },
  { name: 'CCTV Dubai Hills',        href: '/cctv-installation-dubai-hills' },
  { name: 'CCTV JVC',                href: '/cctv-installation-jvc' },
  { name: 'CCTV Al Barsha',          href: '/cctv-installation-al-barsha' },
  { name: 'View all 41 areas',       href: '/areas' },
]
    ...
    <footer style={{ background: '#0F172A', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
      <div className="container-custom py-14">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
          ...
          {/* Popular Areas */}
          <div>
            <h3 className="text-white font-bold font-heading text-sm mb-5 uppercase tracking-wider">Popular Areas</h3>
            <ul className="space-y-2.5">
              {popularAreas.map((a) => (
                <li key={a.name}>
                  <Link href={a.href}
                    className={`text-sm transition-colors flex items-center gap-2 ${a.href === '/areas' ? 'text-blue-300 hover:text-white font-semibold' : 'text-slate-500 hover:text-blue-300'}`}>
                    <span className="w-1 h-1 bg-blue-500 rounded-full flex-shrink-0" /> {a.name}
                  </Link>
                </li>
              ))}
            </ul>
